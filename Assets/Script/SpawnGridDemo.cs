...
/// <summary>
/// Demo that tests SpawnGrid logic.
/// Creates a random walk through the grid, marking cells as occupied
/// and drawing them as cubes with Gizmos. Used only for debugging.
/// </summary>
public class SpawnGridDemo : MonoBehaviour
{
    ...
    /// <summary>
    /// Creates a fresh grid and performs a random walk from the center,
    /// </summary>
    public void Rebuild()
    {
        grid = new SpawnGrid(gridSize);
        half = new Vector3((gridSize.x-1)/2f, (gridSize.y-1)/2f, (gridSize.z-1)/2f);

        // Starts walk from the center
    ...
    /// <summary>
    /// Draws all occupied cells as semi-transparent cubes in the Scene view.
    /// </summary>
    void OnDrawGizmos()
    {
        if (grid == null) return;
